...
// method sets the pixel format and size of the image used by the video capturer, by setting properties of the OTVideoFormat object.
...
//method call to release the capture
- (void)releaseCapture {
    [self stopCapture];
...
//The implementation of the setupAudioVideoSession method uses the AVFoundation framework to set the camera to capture images. In the first part of the method an instance of the AVCaptureVideoDataOutput is used to produce image frames.The frames captured with this method are accessed with the [AVCaptureVideoDataOutputSampleBufferDelegate captureOutput:didOutputSampleBuffer:fromConnection:] delegate method. The AVCaptureDevice object represents the camera and its properties. It provides captured images to an AVCaptureSession object.
...
// set the black frame behind the video frame
- (void)initBlackFrameSender {
    _blackFrameTimer = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER,
                                              0, 0, _capture_queue);
    int blackFrameWidth = [[UIScreen mainScreen]bounds].size.width;
    int blackFrameHeight = [[UIScreen mainScreen]bounds].size.height-50;
...
//method is called when the publisher captured video to publish.
- (BOOL) isCaptureStarted {
    return (_captureSession || _blackFrameTimer) && _capturing;
}

//method is called when the publisher starts capturing video to publish.
- (int32_t) startCapture {
    _capturing = YES;
    return 0;
}
//method is called when the publisher stops capturing video to publish.
- (int32_t) stopCapture {
    _capturing = NO;
    return 0;
}

//method queries the orientation of the image in AVFoundation framework and returns its equivalent defined by the OTVideoOrientation enum in OpenTok iOS SDK.
...
/*!
 ...
 @brief delegate method is called when a new video frame is available from the camera.
...
Writes image data from two planes into one member buffer. Since the image is an NV12, its data is distributed over two planes. There is a plane for Y data and a plane for UV data. A for loop is executed to iterate through both planes and write their data into one memory buffer.
...
Calls the  consumeFrame method, passing in the OTVideoFrame object. This causes the publisher to send the frame in the stream it publishes. 
